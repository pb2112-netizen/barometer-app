...
core/         Level (pasmo ze score), Tone (sentyment, default NEUTRAL — WB-014),
              ...
              Sparkline + SparklineBitmap (WB-003: wykres 72h, wspólna logika dashboard+widget),
              LegalLinks, OpenExternal (stałe URL/mail, Intent bez WebView)
data/model/   BarometerData, ScoreHistoryPoint, TopEvent (@Serializable, 1:1 z barometer.json)
...
- **Poziomy/kolory (dwuosiowe od v0.7.0, WB-014/015):** pasmo = `Level.fromScore(score)`
  ...
  **Sparkline 72h (WB-003):** z `score_history[]` w JSON; Y stałe 1–10; puls ostatniego punktu
  tylko w apce gdy dane świeże i ≥3 punkty; widget — mini bitmap bez animacji.
  Pole `trend` w JSON nadal dla a11y/fallback (nie rysowane jako strzałka od v0.8.0).
...
dla apek ≤ v0.6.x; nowa apka ignoruje). `score_history[]` (WB-003): `{t, s}` rolling 72 h.
